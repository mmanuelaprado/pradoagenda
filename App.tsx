...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const init = () => {
      ...
      const protectedRoutes = ['dashboard', 'login', 'signup', 'agenda', 'services', 'clients', 'company', 'settings', 'inactivation', 'recurring', 'apps', 'finance', 'favicon.ico', 'manifest.json', 'sw.js'];
  ...
  const handleSaveAppointment = async (appt: Omit<Appointment, 'id'>) => {
    ...
    if (user?.id === profId) fetchInitialData(profId);
  };
